...
def get_old_paper_set():
    if os.path.exists(old_paper_set_path):
        with open(old_paper_set_path, "rb") as fp:
            old_paper_set = pickle.load(fp)
    else:
        old_paper_set = set()
    return old_paper_set
...
def main():
    ...
    # 4. 요약된 내용을 JSON 파일로 저장합니다.
    # fields = set()
    # for workspace in WORKSPACES:
    #     for field in workspace["fields"]:
    #         fields.add(field)

    fields = FIELDS  # settings.py에 정의된 fields 사용
    ...
    paper_summarizations = get_paper_summarizations()
    old_paper_set = get_old_paper_set()
    summaries = []
    ...
    for field in fields:
        ...
        for paper_url, paper_title, paper_comment in new_papers[field]:
            ...
            with open(old_paper_set_path, "wb") as fp:
                pickle.dump(old_paper_set, fp)

    summary_file_path = os.path.join(today_summaries_dir, "summaries.json")
    with open(summary_file_path, "w", encoding="utf-8") as json_file:
    ...
    #     # save to JSON
    #     print("Saving summaries to JSON files...")
    #     for thread in threads:
    #         summary_file_path = os.path.join(today_summaries_dir, workspace_name + ".json")
    #         summaries = []
